...
class Esheet{
    map: Map<string, Ecell>;

    constructor(){
        this.map = new Map<string, Ecell>();
    }
...
class Ecell {
    ...
    constructor(loc: string, rawValue: string, map: Map<string, Ecell>){
        this.location = loc;
        this.rawValue = rawValue;
        this.value = this.put(rawValue, map);
    }

    put(rawValue: string, map: Map<string, Ecell>): number{
        let newParent = []
...
function testExcel2(){
    let Spreadsheet: Esheet = new Esheet();
